...
<body>
	...
	<form method="POST" class="info-form" enctype="multipart/form-data" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
		...
		<input type="text"  autocomplete="off" placeholder="Name of the Game" value="" name="gamename"     class="game-input chosen-value" required>
		<input type="hidden" name="gid" class="gamid">
		<ul class="value-list">
		...
		<input type="text" name="players" value="<?php echo $_SESSION["gameArray"]["num"]?>" placeholder="Maximum number of players (like 5,10 or 20)" required>
		<?php echo $numerr ?> 
		<input type="text" value="<?php echo $_SESSION["gameArray"]["skills"] ?>" name="things" placeholder="Things required (like: racquet, bat)">             
		...
		<textarea rows="2" cols="55" wrap="hard" name="describe" placeholder="Anything else you want to mention?"><?php $_SESSION["gameArray"]["describe"]?></textarea>	
